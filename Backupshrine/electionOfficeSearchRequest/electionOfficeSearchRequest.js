...
import getElectionOffices from '@salesforce/apex/ElectionOfficeController.getElectionOffices';
import getElectionOfficesCount from '@salesforce/apex/ElectionOfficeController.getElectionOfficesCount';
import getPublicOfficial from '@salesforce/apex/ElectionOfficeController.getPublicOfficial';
import getPublicOfficialsCount from '@salesforce/apex/ElectionOfficeController.getPublicOfficialsCount';
import getOfficeOptions from '@salesforce/apex/ElectionOfficeController.getOfficeOptions';
import deleteContact from '@salesforce/apex/PublicOfficialController.deleteContact';
import publicOfficialModal from 'c/publicOfficialModal';
import electionOfficeModal from 'c/electionOfficeModal';
            ...
            .then(result => {
                this.paginatedResult = result.map(wrapper => {
                    ...
                    return {
                        ...
                        Id__c: wrapper.office.Id__c != null && String(wrapper.office.Id__c).length < 3
                            ? String(wrapper.office.Id__c).padStart(3, '0')
                            : String(wrapper.office.Id__c),
                        Name__c: this.toTitleCase(wrapper.office.Name__c || ''),
                        Title__c: this.toTitleCase(wrapper.office.ROV_Name__c || ''),
                        Mailing_Address_Line_1__c: this.toTitleCase(wrapper.office.Mailing_Address_Line_1__c || ''),
                        Mailing_Address_City__c: this.toTitleCase(wrapper.office.Mailing_Address_City__c || ''),
                        Mailing_Address_State__c: wrapper.office.Mailing_Address_State__c,
                        Mailing_Address_Zip__c: wrapper.office.Mailing_Address_Zip__c != null && String(wrapper.office.Mailing_Address_Zip__c).trim() !== '' && String(wrapper.office.Mailing_Address_Zip__c).length < 5
                        ? String(wrapper.office.Mailing_Address_Zip__c).padStart(5, '0')
                        : wrapper.office.Mailing_Address_Zip__c != null && String(wrapper.office.Mailing_Address_Zip__c).trim() !== ''
                            ? String(wrapper.office.Mailing_Address_Zip__c)
                            : '',
                        Election_Held_In__c: wrapper.office.Election_Held_In__c,
...
// Convert the object to a JSON string
    ...
    get sortIcons() {
        return {
            Name__c: this.sortedBy === 'Name__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            Id__c: this.sortedBy === 'Id__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            Election_Held_In__c: this.sortedBy === 'Election_Held_In__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            Title__c: this.sortedBy === 'ROV_Name__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            Mailing_Address_Line_1__c: this.sortedBy === 'Mailing_Address_Line_1__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            Mailing_Address_City__c: this.sortedBy === 'Mailing_Address_City__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            Mailing_Address_State__c: this.sortedBy === 'Mailing_Address_State__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            Mailing_Address_Zip__c: this.sortedBy === 'Mailing_Address_Zip__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
    ...
    get sortedClassId(){
        return `slds-is-sortable ${this.sortedBy === 'Id__c' ? 'sorted' : ''}`;

    }

    get sortedClassName(){
        return `slds-is-sortable ${this.sortedBy === 'Name__c' ? 'sorted' : ''}`;

    }

    get sortedClassAddress(){
        return `slds-is-sortable ${this.sortedBy === 'Mailing_Address_Line_1__c' ? 'sorted' : ''}`;

    }
    get sortedClassCity(){
        return `slds-is-sortable ${this.sortedBy === 'Mailing_Address_City__c' ? 'sorted' : ''}`;

    }

    get sortedClassState(){
        return `slds-is-sortable ${this.sortedBy === 'Mailing_Address_State__c' ? 'sorted' : ''}`;

    }

    get sortedClassZip(){
        return `slds-is-sortable ${this.sortedBy === 'Mailing_Address_Zip__c' ? 'sorted' : ''}`;

    }

    get sortedClassTitle(){
        return `slds-is-sortable ${this.sortedBy === 'ROV_Name__c' ? 'sorted' : ''}`;

    }

    get sortedClassElection(){
        return `slds-is-sortable ${this.sortedBy === 'Election_Held_In__c' ? 'sorted' : ''}`;
    ...
    applyFilters() {
        ...
        .then(result => {
          ...
          this.paginatedResult = result.map(wrapper => {
            return {
                Id: wrapper.office.Id,
                Id__c: wrapper.office.Id__c,
                Name__c: wrapper.office.Name__c,
                Mailing_Address_Line_1__c: wrapper.office.Mailing_Address_Line_1__c,
                Mailing_Address_City__c: wrapper.office.Mailing_Address_City__c,
                Mailing_Address_State__c: wrapper.office.Mailing_Address_State__c,
                Mailing_Address_Zip__c: wrapper.office.Mailing_Address_Zip__c != null && String(wrapper.office.Mailing_Address_Zip__c).trim() !== '' && String(wrapper.office.Mailing_Address_Zip__c).length < 5
                ? String(wrapper.office.Mailing_Address_Zip__c).padStart(5, '0')
                : wrapper.office.Mailing_Address_Zip__c != null && String(wrapper.office.Mailing_Address_Zip__c).trim() !== ''
    ...
    async openSealAddModal() {
        this.isStaffModalOpen = true;
         ...
         await electionOfficeModal.open({
            size: 'small',
    ...
    async deleteRequest(recordId){
         ...
         await deleteConfirmationModal.open({
            size: 'small',
            description: 'Accessible description of modal\'s purpose',
            recordId: recordId,
            officeType: this.typeofOffice,
    ...
    get officialsortIcons() {
        return {
            Title__c: this.officialSortedBy === 'Title__c' ? (this.officialSortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            Party__c: this.officialSortedBy === 'Party__c' ? (this.officialSortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            ...
            Start_Term__c: this.officialSortedBy === 'Start_Term__c' ? (this.officialSortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            End_Term__c: this.officialSortedBy === 'End_Term__c' ? (this.officialSortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            ...
            Extension__c: this.officialSortedBy === 'Extension__c' ? (this.officialSortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown'

        };
    }

    get sortedClassOfficialTitle(){
        return `slds-is-sortable ${this.officialSortedBy === 'Title__c' ? 'sorted' : ''}`;

    }

    get sortedClassParty(){
        return `slds-is-sortable ${this.officialSortedBy === 'Party__c' ? 'sorted' : ''}`;
    ...
    get sortedClassStartTerm(){
        return `slds-is-sortable ${this.officialSortedBy === 'Start_Term__c' ? 'sorted' : ''}`;
    }

    get sortedClassEndTerm(){
        return `slds-is-sortable ${this.officialSortedBy === 'End_Term__c' ? 'sorted' : ''}`;
    }
    ...
    get sortedClassExtension(){
        return `slds-is-sortable ${this.officialSortedBy === 'Extension__c' ? 'sorted' : ''}`;
    }
    ...
    async openSealAddOfficialModal(){
         ...
         await publicOfficialModal.open({
            size: 'small',
            description: 'Accessible description of modal\'s purpose',
    ...
    handleNavigateToAdd(){
        ...
        this[NavigationMixin.Navigate]({
            ...
            state: {
                ...
                c__returnTo: 'c__electionOfficeSearchRequest',
    ...
    handleExportResultButtonClick(){
        ...
        this.template.querySelector('c-public-official-export-to-excel').exportDataToExcelElectionOffice(headers, searchCriteria, fileName);
    ...
    handleExportResultButtonOfficialClick(){
        ...
        this.template.querySelector('c-public-official-export-to-excel').exportDataToExcelElectionOfficial(headers, searchCriteria, fileName);
