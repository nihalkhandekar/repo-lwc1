...
import getActivityTransactionData from '@salesforce/apex/SAP_BatchFinsysControllerDuplicate.getActivityTransactionDataWhenOnlyIdIsAvailable';
import UpdateBatchStatus from '@salesforce/apex/SAP_BatchFinsysControllerDuplicate.UpdateBatchStatus';
...
export default class ViewOrEditBatchFinsys extends NavigationMixin(LightningModal) {
    ...
    if (data) {
      ...
      if (batchData.Fees) {
        batchData.Fees = batchData.Fees.map((item) => ({
          ...
          transactionType: item.TransactionType === 'Refund_Transaction' ? 'Refund' : ''
        }));
      }
  ...
  connectedCallback() {
    loadStyle(this, sap_stateExtradition).catch((error) => {
  ...
  goBackModalToComponent() {
    this[NavigationMixin.Navigate]({
      type: 'standard__component',
      attributes: {
        componentName: 'c__sap_BatchFinsys'
      }
    });
  }
  ...
  handlePrintBatch() {
    const sap_PdfGenerator = this.template.querySelector('c-sap_-pdf-genrator');
    if (pdfGenerator) {
      sap_PdfGenerator.generateBatchPDF();
    }
  }
}