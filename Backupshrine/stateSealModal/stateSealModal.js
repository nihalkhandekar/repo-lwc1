...
import upsertApplicationStateSealArm from '@salesforce/apex/StateSealApplicationController.upsertApplicationStateSealArm';
import fetchApplicationData from '@salesforce/apex/StateSealApplicationController.fetchApplicationData';
...
export default class stateSealModal extends NavigationMixin(LightningElement) {
    ...
    connectedCallback() {
        ...
        .then(() => {
            console.log('First CSS file (modalStateSealRequest) loaded successfully');
        })
    ...
    async fetchExistingData() {
      return  fetchApplicationData({ recordId: this.recordId })
            .then((data) => {
                ...
                this.formData = {
                    ...this.formData,
                    prefix: data.Prefix__c,
                    lastName: this.toTitleCase(data.Last_Name__c || ''),
                    middleInitial: this.toTitleCase(data.Middle_Name__c || ''),
                    firstName: this.toTitleCase(data.First_Name__c || ''),
                    address1: this.toTitleCase(data.Address_Line_1__c || ''),
                    city: this.toTitleCase(data.City__c || ''),
                    suffix: data.Suffix__c,
                    dob: data.Date_Of_Birth__c,
                    title: data.Title__c,
                    entity: data.Organization_Name__c,
                    email: data.Email_Address__c,
                    phoneNumber: data.Cell_Phone_Number__c,
                    esq: data.ESQ__c,
                    address2: data.Suite_Apartment_Floor__c,
                    state: data.State__c,
                    country: data.Country__c,
                    zipCode: data.Zip_Code__c,
                    letterText: data.Letter_Text__c,
                    enclosure: data.Enclosure__c,
                    proposedUse: data.Proposed_Use__c,
                    requestedFor: data.Requested_For__c,
                    disposition: data.Disposition__c,
                    approvedFor: data.Approved_For__c,
                    dateOfResponse: data.Date_of_SOTS_Response__c,
                    reason: data.Reason__c,
                    letterType: data.Letter_Type__c,
                    signedBy: data.Signedby__c,
                    wetSign: data.Wet_Signature__c,
        ...
        if(this.mode === 'edit'){
             ...
             try {
                 ...
                 this[NavigationMixin.Navigate]({
                     ...
                     attributes: {
                         componentName: 'c__stateSealArmsRequest'  // The target component name
                     }
                 });
