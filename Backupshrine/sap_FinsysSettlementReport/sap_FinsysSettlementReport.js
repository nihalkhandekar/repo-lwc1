...
import getSettlementCount from '@salesforce/apex/SAP_FinsysSettlementReport.getSettlementCount';
import getSettlementData from '@salesforce/apex/SAP_FinsysSettlementReport.getSettlementData';
...
export default class SAP_FinsysSettlementReport extends LightningElement {
    ...
    @track workOrder = '';
    @track dateCreated = new Date(new Date().getTime() - new Date().getTimezoneOffset() * 60000)
    .toISOString()
    .slice(0, 10);
    @track amount = null;
    @track receivedBy = '';
        ...
        fields.forEach((field) => {
            switch (field.label) {
                ...
                case 'Work Order#':
                    ...
                    break;
                case 'Date':
                    this.dateCreated = field.value || new Date().toISOString().slice(0, 10);
                    break;
                case 'Amount':
