...
import INDVIDUALAPP_SUFFIX_FIELD from '@salesforce/schema/IndividualApplication.Suffix__c';
import EXTRADITED_FROM_FIELD from '@salesforce/schema/IndividualApplication.ExtraditedFrom__c';
import upsertNewRequestData from '@salesforce/apex/ExtraditionRequestController.upsertNewRequestData';
import fetchExtradictionrequestData from '@salesforce/apex/ExtraditionRequestController.fetchExtradictionrequestData';
import getAttorneyInspectorOptions from '@salesforce/apex/MaintainStaffDataController.getAttorneyInspectorOptions';
import ExtraditionRequestEmailOption from 'c/extraditionRequestEmailOption';
...
export default class ExtradictionRequestModal extends NavigationMixin(LightningModal) {
    ...
    @track notes;
    @track attorneyInspectorOptions = [];
    // @track officeOptions = [];
    @track receivedForFillingDate;
    @track extractedFileNumber;
    @track attorneyInspector;
    @track office;
    @api comments;
    @track location = '';
    @track address2 = '';
    ...
    // Fetch picklist values for ExtraditedFrom__c
    @wire(getPicklistValues, {
      ...
      return  fetchExtradictionrequestData({ recordId: this.idofrecord })
            .then((data) => {
                ...
                if (data) {
                    ...
                    this.extractedFileNumber = data.Extradicted_File_Number__c;
                    this.attorneyInspector = data.Attorney_Inspector__c;
                    this.office = this.toTitleCase(data.Office__c || ''),
                    this.location = data.Address_Line_1__c || '';
                    this.address2 = data.Suite_Apartment_Floor__c;
                    this.city = data.City__c;
                    this.state = data.State__c;
                    this.zipCode = data.Zip_Code__c;
                    this.country = data.Country__c;
                    this.comments = data.Comments_for_SOTS_use_only__c;
                    this.extradictedFrom = data.Extradicted_From__c;
                    this.esq = data.ESQ__c ? 'true' : 'false';
                    this.lastName = data.Last_Name__c;
                    this.middleInitial = data.Middle_Name__c;
                    this.firstName = data.First_Name__c;
                    this.suffix = data.Suffix__c;
                    this.requestDate = data.Request_Date__c;
                    this.responseDate = data.Response_Date__c;
                    this.extradictionpc = data.Extradiction_PC__c;
                    this.extradictionAuth = data.Extradiction_Auth__c;
                    this.notes = data.Notes_on_Receipt__c;
                    this.recordTypeId = data.RecordTypeId;
                    this.accountId = data.AccountId;
                    this.licenseTypeId = data.LicenseTypeId;
                    this.destination = data.Destination__c;
                    if (data.Persons_AKA__r) {
                        this.akaList = data.Persons_AKA__r.map(aka => ({
                            id: aka.Id,
                            akalastName: aka.Name || '',
                            akamiddleInitial: aka.Middle_Name__c || '',
                            akafirstName: aka.First_Name__c || '',
                            akasuffix: aka.Suffix__c || '',
                            akaesq: aka.ESQ__c || false
                        }));
                    }
                } else {
        ...
        if(isValid){
            try {
                const requestData = {
                    ...
                    attorneyInspector: this.attorneyInspector,
                    office: this.office,
        ...
        if(this.mode === 'edit'){
             ...
             try {
                 ...
                 this[NavigationMixin.Navigate]({
                     ...
                     attributes: {
                         componentName: 'c__stateExtraDictionsRequests'  // The target component name
                     }
                 });
