...
import fetchSearchResultsUpdated from '@salesforce/apex/SAP_WorkOrderCheckProcessing.fetchSearchResultsUpdated';
import getSearchResultsCountUpdated from '@salesforce/apex/SAP_WorkOrderCheckProcessing.getSearchResultsCountUpdated';
import logDropdownChange from '@salesforce/apex/SAP_WorkOrderCheckProcessing.logDropdownChange';
import logCheckboxChange from '@salesforce/apex/SAP_WorkOrderCheckProcessing.logCheckboxChange';
import uploadFile from '@salesforce/apex/SAP_WorkOrderCheckProcessing.uploadFile';
import deleteFile from '@salesforce/apex/SAP_WorkOrderCheckProcessing.deleteFile';
...
export default class FinsysWorkOrderCheckProcessing extends LightningElement {
        ...
        try {
            ...
            this.searchResults = data.map(record => {
                const documents = record.documents || []; 
                    return {
                        ...record,
                        paymentAmount: this.formatPaymentAmount(record.paymentAmount), 
    ...
    handleUpload2() {
        ...
        this.closeUploadModal2(); 
    }
    

    handleDeleteFile(event) {
        const rowId = event.target.dataset.id; 
        const indexToDelete = parseInt(event.target.dataset.id, 10); 
    ...
    sortByField(event) {
        const fieldToSortMap = {
            ...
            lastName: 'Activity__r.Individual_Application__r.SAP_Last_Name__c',
            firstName: 'Activity__r.Individual_Application__r.SAP_First_Name__c',
        ...
        this.sortedBy = fieldToSortMap[selectedField] || 'CreatedDate'; 
    ...
    get sortIcons() {
        return {
            ...
            lastName: this.sortedBy === 'Activity__r.Individual_Application__r.SAP_Last_Name__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            firstName: this.sortedBy === 'Activity__r.Individual_Application__r.SAP_First_Name__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
    ...
    get sortedlastName(){
        return `slds-is-sortable ${this.sortedBy === 'Activity__r.Individual_Application__r.SAP_Last_Name__c' ? 'sorted' : ''}`;

    }

    get sortedfirstName(){
        return `slds-is-sortable ${this.sortedBy === 'Activity__r.Individual_Application__r.SAP_First_Name__c' ? 'sorted' : ''}`;
