...
import LETTERTYPE_FIELD from '@salesforce/schema/Contact.SAP_letterType__c';
import WETSIGN_FIELD from '@salesforce/schema/Contact.SAP_Wet_Signature__c';
import JUDICIAL_DISTRICT_FIELD from '@salesforce/schema/Contact.SAP_Judicial_District__c';
import getLatestHistoryRecords from '@salesforce/apex/SAP_HistoryTrackerController.getLatestHistoryRecords';
import fetchPublicOfficialData from '@salesforce/apex/SAP_ExtraditionPublicOfficialController.fetchPublicOfficialData';
import updatePublicOfficialData from '@salesforce/apex/SAP_ExtraditionPublicOfficialController.updatePublicOfficialData';
import getStateSealStaffData from '@salesforce/apex/SAP_StateSealApplicationController.getStateSealStaffData';
import processNameFieldHistory from '@salesforce/apex/SAP_InHousePublicOfficialController.processNameFieldHistory';
import processTermFieldHistory from '@salesforce/apex/SAP_InHousePublicOfficialController.processTermFieldHistory';
...
export default class StateExtraditionModal extends NavigationMixin(LightningModal) {
     ...
     return  fetchPublicOfficialData({ recordId: this.idofrecord })
            .then((data) => {
                console.log('Data coming from Apex:', JSON.stringify(data));
                    ...
                    this.prefix = data.records[0]?.SAP_Prefix__c,
                    this.middleInitial = data.records[0]?.MiddleName,
                    this.esq = data.records[0]?.SAP_Esquire__c,
                    this.firstName = data.records[0]?.FirstName;
                    this.suffix = data.records[0]?.Suffix,
                    this.termstartDate = data.records[0]?.SAP_Start_Term__c,
                    this.termendDate = data.records[0]?.SAP_End_Term__c,
                    this.position = data.records[0]?.SAP_Position__c,
                    this.GA = data.records[0]?.SAP_GA__c,
                    this.isIndefiniteTerm = data.records[0]?.SAP_Indefinite_Term__c,
                    this.judicialDistrict = data.records[0]?.SAP_Judicial_District__c,
                    this.SealStampof = data.records[0]?.SAP_Seal_Stramp_of__c,
                    this.Notes = data.records[0]?.SAP_Notes__c, 
                    this.letterType = data.records[0]?.SAP_letterType__c,
    ...
    async fetchTermHistory() {
        ...
        .then(result => {
            ...
            this.historyTermRecords = result.map(record => {
                return {
                    changeDate: record.changeDate, 
                    endTerm: record.SAP_End_Term__c || '',
                    startTerm: record.SAP_Start_Term__c || ''
                };
            });
        })
            .catch((error) => {
    ...
    async handleAdd() {
        ...
        if (isValid && !this.showError && isValidMsg) {
            const data = {
                SAP_Prefix__c: this.prefix,
                FirstName: this.firstName,
                MiddleName: this.middleInitial,
                LastName: this.lastName,
                SAP_Suffix__c: this.suffix,
                SAP_Start_Term__c: this.termstartDate,
                SAP_End_Term__c: this.termendDate,
                SAP_Position__c: this.position,
                SAP_GA__c: this.GA,
                SAP_Indefinite_Term__c: this.isIndefiniteTerm,
                SAP_Judicial_District__c: this.judicialDistrict,
                SAP_Seal_Stramp_of__c: this.sealstamp,
                SAP_Notes__c: this.notes,
                SAP_letterType__c: this.letterType,
                SignedBy__c: this.signedBy,
                SAP_Wet_Signature__c : this.wetSign,
                SAP_Esquire__c : this.esq,
                Id: this.idofrecord
            };
             ...
             try {
                ...
                this.endDateErrorMessage='';
                 ...
                 this[NavigationMixin.Navigate]({
                     ...
                     attributes: {
                         componentName: 'c__sap_SearchOfficials'  // The target component name
                     }
                 });
    ...
    ***************************************************/
    ...
    wiredHistoryRecords({ data, error }) {
        if (data) {
            ...
            this.prevfirstName = data.SAP_First_Name__c?.OldValue || '';
            this.prevlastName = data.SAP_Last_Name__c?.OldValue || '';
            this.prevmiddleName = data.SAP_Middle_Name__c?.OldValue || '';
