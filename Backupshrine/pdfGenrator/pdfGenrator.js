...
import mainData from '@salesforce/apex/pdfDataFetch.fetchTransactionData';
import documentRequestImage from '@salesforce/resourceUrl/apostilleDocumentRequestedImageForPrintSubmissionDocument';
import mailingImage from '@salesforce/resourceUrl/apostilleDocumentRequestedImageForMailingInformation';
import jsPdfAutotable from '@salesforce/resourceUrl/jsPdfAutotable';
import fetchEnvelopeData from '@salesforce/apex/PrintDocumentController.fetchEnvelopeData';
import fetchLetterData from '@salesforce/apex/PrintDocumentController.fetchLetterData';
import getBase64Image from '@salesforce/apex/ApostilleLetterController.getBase64Image';
import getBatchPDFData from '@salesforce/apex/BatchPDFController.getBatchData';
...
export default class PdfGenerator extends LightningElement {
    ...
    processData(data) {
        ...
        this.Tax_Exempt = this.RegulatoryTrxnFee1.Tax_Exempt__c || false;
        ...
        this.Customer_Type = this.Account1.Customer_Type__c || '';
        this.Customer_ID = this.Account1.Customer_ID__pc || '';
        this.Organization_Name = this.Account1.Organization_Name__c || '';
    ...
    async LetterCertificatePdfGenerator(certificateNo){
        ...
        doc.setFontSize(10);
        doc.text('Thank you for presenting your documents to the Connecticut Secretary of the State. Please find the order details below:', marginLeft, yPosition, { maxWidth: textWidth });
        yPosition += 10;
        ...
        const paymentType = paymentDetail.Payment_Method__c || '---';
        const partialRefund = '---';
        const refundType = '---';
        ...
        doc.text(infoText, marginLeft, yPosition, { maxWidth: textWidth, lineHeightFactor: 1.15 }); // Tighter line spacing for paragraph
        ...
        const linkUrl = `https://ctds--sapdev001.sandbox.my.site.com/eApostille/apostilleverification?certificateNo=${encodeURIComponent(certificateNo)}`;
        yPosition += 8;
        ...
        const closingText = [
            ...
            'Authentications and Apostille Unit',
            '165 Capitol Ave Suite 100,',
            'Hartford, CT 06106',
            'bsd@ct.gov'
        ];
    ...
    async generateDataForApostillePrintPaymentReceipt() {
        ...
        this.records.forEach((record, index) => {
            ...
            // Adjusted the rightMargin to reduce the gap
            const rightMargin = pageWidth - marginRight - 50; // Closer to the label
            ...
            details.forEach((item, i) => {
                ...
                doc.text(item.value, rightMargin, y); // Adjusted the right margin to reduce gap
            });

            // Add a decorative element (line below details)
            doc.setDrawColor(12, 124, 206);
            doc.setLineWidth(0.5);
            doc.line(leftMargin, startY + (details.length * lineHeight) + 5, pageWidth - marginLeft, startY + (details.length * lineHeight) + 5);

            // Adjust startY for the next record
            yPosition = startY + (details.length * lineHeight) + 5;  // Increase margin for next record
        });
    ...
    @api
    async generateDataForInhouseApostillePrintPaymentReceipt() {
        console.log('API method called:');
        ...
        this.records.forEach((record, index) => {
            ...
            // Add a decorative element (line below details)
            doc.setDrawColor(12, 124, 206);  // Blue color
            doc.setLineWidth(0.5);
            const blueLineYPosition = startY + (details.length * lineHeight) + 5; // Position the line below details
            doc.line(marginLeft, blueLineYPosition, pageWidth - marginRight, blueLineYPosition);  // Blue line

            // Adjust startY for the next record
            yPosition = blueLineYPosition + 5;  // Start next content just below the blue line
        });
        ...
        // **Table width should be exactly the same as the blue line's width**
        // The table should start from marginLeft and end at pageWidth - marginRight (same as blue line's width)
        const tableWidth = pageWidth - marginLeft - marginRight;  // Full width of the page minus margins

        // Add the table with autoTable, starting at the same position as the blue line
        doc.autoTable({
            head: [tableDataHeader],  // Use the defined table header
            body: tableData,
            startY: yPosition,  // Start the table directly after the blue line
            margin: { left: marginLeft, right: marginRight },
            styles: {
                font: 'helvetica',
                fontSize: 10,
            ...
            tableWidth: tableWidth  // Table width is the same as the blue line
        });
...
connectedCallback() {
    ...
    console.log('workOrderNumb:', this.workOrderNumb);
...
async generateDataForPrintApostilleSubmissionDocument() {
    ...
    doc.setFontSize(20);
    doc.setFont('Verdana', 'normal');
    doc.text('Submitted Documents', pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 15;
    ...
    doc.text('Thank You,', marginLeft, yPosition);
    yPosition += 5;
    doc.text('Your apostille request has been successfully submitted. Below are the details of your request:', marginLeft, yPosition);
    yPosition += 10;
    ...
    const linkUrl = `https://ctds--sapdev001.sandbox.my.site.com/eApostille/apostillerequest?workOrderNumber=${encodeURIComponent(this.workOrderNum)}`;
    doc.setTextColor(0, 0, 255);
    doc.text(linkText, marginLeft, yPosition);
    const underlineWidth = doc.getTextWidth(linkText);
    doc.setLineWidth(0.2);
...
doc.line(leftMargin, startY + (details.length * lineHeight) + 5, pageWidth - leftMargin, startY + (details.length * lineHeight) + 5);
    ...
    generateApostilleHouseCertificate() {
        ...
        const tableData = [
            [{ content: "APOSTILLE\n(Convention de La Haye du 5 octobre 1961)", colSpan: 2, styles: { fontStyle: 'bold', halign: 'center' } }],
            [{ content: "1. Country:\n     País:", colSpan: 2 }],
            [{ content: "2. has been signed by\n     ha sido firmado por", colSpan: 2 }],
            [{ content: "3. acting in the capacity of\n     quien actúa en calidad de", colSpan: 2 }],
            [{ content: "4. bears the seal / stamp of\n     y está revestido del sello / timbre de", colSpan: 2 }],
            [{ content: "Certified\nCertificado", colSpan: 2, styles: { fontStyle: 'bold', halign: 'center' } }],
            [{ content: "5. at Hartford, Connecticut\n    en", colSpan: 1 }, { content: "6. the\n    el día", colSpan: 1 }],
            [{ content: "7. by\n     por Stephanie Thomas, Secretary of the State of Connecticut", colSpan: 2 }],
            [{ content: "8. Nº\n     bajo el número 2025 - xxxxx", colSpan: 2 }],
            [{ content: "9. Seal / stamp:\n    Sello / timbre:", colSpan: 1 }, { content: "10. Signature:\n       Firma:", colSpan: 1 }]
        ];
