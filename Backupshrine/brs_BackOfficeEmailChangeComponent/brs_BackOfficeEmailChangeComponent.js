...
export default class Brs_BackOfficeEmailChangeComponent extends LightningElement {
  ...
  connectedCallback() {
    ...
    // if (this.originalEmailId && !this.val) {
    //   this.val = this.originalEmailId.slice();
    //   this.confirmval = this.originalEmailId.slice();
    //   this.checkPassword();
    // }
  ...
  handleCustomValidation(val) {
   // const emailPattern = /^[A-zA-z0-9]{1}[\w-]*@(?![0-9]*\.)[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9]{2,})$/;
  ...
  handleEmailEdit(){
    ...
    if(this.showUpdateLabel){
      ...
      // this.passUpdatedValueToFlow();
    }
  }



passUpdatedValueToFlow  () {
    const attributeChangeEventError = new FlowAttributeChangeEvent('varChangeEmailValue', this.val);
