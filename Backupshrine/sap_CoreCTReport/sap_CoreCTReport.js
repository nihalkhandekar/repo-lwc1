...
export default class CoreCTReport extends LightningElement {
    ...
    fields.forEach((field) => {
      if (field.label === "Select Report Type") {
        if (field.value) {
          ...
          if (
            this.isRegularDeposits !==
            (field.value === "Regular Bank Deposits")
          ) {
  ...
  createMergedData(result) {
    ...
    this.hasBRSRecords = mergedData.length > 0;
    return mergedData;
  }
  ...
  get totalRow() {
    return this.brsTransactions.reduce(
      (totals, record) => {
        totals.MasterCardAmount += record.MasterCardAmount || 0;
        totals.VisaAmount += record.VisaAmount || 0;
        totals.AmexAmount += record.AmexAmount || 0;
        totals.DiscoverAmount += record.DiscoverAmount || 0;
        totals.AmericanExpressAmount += record.AmericanExpressAmount || 0;
        totals.JCBAmount += record.JCBAmount || 0;
        totals.DinersClubAmount += record.DinersClubAmount || 0;
        totals.UnknownAmount += record.UnknownAmount || 0;
        totals.TotalAmount += record.TotalAmount || 0;
        return totals;
      },
      {
        MasterCardAmount: 0,
  ...
  processRegularDepositsData(result) {
    ...
    this.paymentSections = this.paymentSections.map((section) => {
      ...
      const totalAmount = sectionRecords.reduce((sum, record) => {
        return (
          sum +
          (record.children.length > 0
            ? record.children[0].FeeAmount__c || 0
            : 0)
        );
      }, 0);
  ...
  handleDownloadPdf() {
    ...
    const pdfgenerator = this.template.querySelector(
      "c-sap_-finsys-reports-pdf-generator"
    );
    if (pdfgenerator) {
      if (this.isRegularDeposits) {
        pdfgenerator.coreRegularDepositReport(params, fileName);
      } else {
        pdfgenerator.coreCreditCardReport(params, fileName);
      }
    } else {
      console.error("Excel generator component not found");
    }
