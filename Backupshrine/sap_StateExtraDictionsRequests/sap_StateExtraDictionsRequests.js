...
import getStateExtradictionApplications from '@salesforce/apex/SAP_IAStExtradictionController.getStateExtradictionApplications';
...
export default class StateExtraDictionsRequests extends NavigationMixin(LightningElement) {
    ...
    formatTableData(records) {
        ...
        return records.map(row => ({
            ...row,
            SAP_Last_Name__c: row.SAP_Last_Name__c || '-',
            SAP_First_Name__c: row.SAP_First_Name__c || '-',
            SAP_Middle_Name__c: row.SAP_Middle_Name__c || '-',
            SAP_Extradicted_From__c: row.SAP_Extradicted_From__c || '-',
            SAP_Extradicted_File_Number__c: row.SAP_Extradicted_File_Number__c || '-',
    ...
    async viewRequest(recordId) {
        // const result = await sap_ExtradictionRequestModal.open({
        //     size: 'small',
        ...
        try {
            ...
            this[NavigationMixin.Navigate]({
                ...
                attributes: {
                    componentName: 'c__sap_ExtradictionRequestModal'  // The target component name
                },
                state: {
                  c__record: recordId,
                  c__mode:'view'
