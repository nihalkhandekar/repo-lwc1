...
export default class RegistrationRequirement extends LightningElement {
  ...
  connectedCallback() {  
    ...
    if(this.comparisonCheck && !this.isMaintenance)
    {
      compareCategories({
  ...
  @api
    validate() {
      if(!this.isNaicsChanged && this.isNaicsChanged != undefined)
      {
        ...
        return {
            isValid: false,
            errorMessage: this.label.BRS_Category_Update_Error
        };
      }
      else if (this.required && !this.selectedcategory) {
            this.showErrorMessage = true;
            fireEvent(this.pageRef, "flowvalidation", {
                detail: { isValid: false }
            });
  ...
  handleNotification(event) {
    if (event.detail.isValid == undefined || event.detail.isValid == true) {
      ...
      if(this.required && !this.selectedcategory){
        // old/previous naics code populates again with the edit button fix
...
if(this.categories){
  ...
  handleBrowseAdd() {
    if (this.finalCategoryId1) {
      ...
      this.selectedcategory= tempObj.value;
      if(this.comparisonCheck)
      {
        compareCategories({
          categoryName: tempObj.value,
          language:this.language 
        }).then((result) => {
          this.isNaicsChanged=result;
        })
  ...
  handleSearchTermChange(event) {
    ...
    capsvalue =  inp.value.toUpperCase();
          ...
          this.insertAnalyticsEvent('NAICS Search',
          inp.value,
          'NAICS Search Term',capsvalue,
