...
import fetchMailingLabelData from '@salesforce/apex/UpdateMailingController.fetchMailingLabelData';
import updateMailingLabeData from '@salesforce/apex/UpdateMailingController.updateMailingLabeData';
...
connectedCallback() {
    Promise.all([
        ...
        console.log('data in the connected call back');
    }).catch(error => {
...
loadUpdateMailingData(){
    ...
    .then((data) => {
        console.log('Mailing Data coming from Apex:', JSON.stringify(data));

        this.lastName = data.records[0]?.Last_Name__c || '';
        this.firstName = data.records[0]?.First_Name__c || '';
        this.middleName = data.records[0]?.Middle_Name__c || '';
        ...
        this.entity = data.records[0]?.Entity__c || '';
        this.numberOfLabel = data.records[0]?.Number_Of_Label__c || '';
        this.fax = data.records[0]?.Fax__c || '';
        this.formattedFax = this.formatPhoneNumber(this.fax);
        this.pager = data.records[0]?.Pager__c || '';
        this.formattedPager = this.formatPhoneNumber(this.pager);
        this.title = data.records[0]?.Title__c || '';
        this.otherinfo = data.records[0]?.Other_Info__c || '';
        this.address1 = data.records[0]?.Address1__c || '';
        this.address2 = data.records[0]?.Address2__c || '';
        this.city = data.records[0]?.City__c || '';
        this.state = data.records[0]?.State__c || '';
        this.country = data.records[0]?.Country__c || '';
        this.zipCode = data.records[0]?.Zip_Code__c || '';
...
handleSearch() {
    ...
    renderedCallback() {
        if(this.anythingUpdated){
            this.loadUpdateMailingData();
            console.log('this is the rendered call back');
