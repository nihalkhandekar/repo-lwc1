...
import getApplications from '@salesforce/apex/ApostilleSubmittedRequestController.getApplications';
import getApplicationsCount from '@salesforce/apex/ApostilleSubmittedRequestController.getApplicationsCount';
import updateApplicationStatusToCancelled from '@salesforce/apex/ApostilleSubmittedRequestController.updateApplicationStatusToCancelled';
...
{ label: 'Translated Document', value: 'Translated Document' }
    ...
    // Fetch current page reference and update application ID
    @wire(CurrentPageReference)
    updateIdFromUrlEveryTime(pageRef) {
        if (Object.keys(pageRef.state).length !== 0) {
            this.resetPagination();
            this.individualApplicationId = pageRef.state.recordId; // Access the recordId parameter
            setTimeout(() => {
                this.handleClear();
            }, 1000);
    ...
    loadApplications() {
        ...
        .then(result => {
            if (result.length > 0) {
                this.searchResult = [...this.searchResult, ...result.filter(newItem => !this.searchResult.some(existingItem => existingItem.Id === newItem.Id)).map(item => {
                    const hasDocuments = item.hasDocuments;

                    // Map through the documents and set the status class for each document
                    ...
                    // Return the modified item with additional properties like actionLabel
    ...
    getActionLabel(appStatus) {
        ...
        const viewStatuses = ['Approved', 'Rejected', 'Cancelled', 'Denied', 'In Review', 'Completed', 'Order Completed - Mail', 'Order Completed – Pick Up'];
    ...
    handleInputChange(event) {
        ...
        if (field === 'documentType') {
            ...
            console.log('selectedDocument',this.selectedDocumentType);
    ...
    async handleApprovedAction(recordId) {
        ...
        try{
            ...
            // Re-enable scrolling when modal closes (whether success or cancel)
    ...
    handleYesOfCancelRequest() {
        console.log('Handling cancel request', this.selectedRecordId);
        updateApplicationStatusToCancelled({ recordId: this.selectedRecordId })
            .then(result => {
                if (result) {
                    this.handleClear();
                }
