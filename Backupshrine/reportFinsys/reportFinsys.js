...
import fetchUserNames from "@salesforce/apex/ReportFinsysController.fetchUserNames";
...
// import agingReport from '@salesforce/apex/ReportDownloadController.agingReport';
// import agingReportCount from '@salesforce/apex/ReportDownloadController.agingReportCount';
...
//     "Work Order Number": {
//       fieldName: "IndividualApplication.Sequence_Number__c",
//       type: "text"
//     },
//     "Customer Name": {
//       fieldName: "IndividualApplication.First_Name__c",
//       type: "text"
//     },
//     "Check Number": {
...
//     },
//     Status: {
//       fieldName: "RegulatoryTrxnFeeItem.Status__c",
//       type: "picklist"
//     },
//     "Reason for Return": {
...
//     },
//     "Customer ID": {
//       fieldName: "RegulatoryTrxnFeeItem.Customer_ID__c",
//       type: "text"
//     },
//     "Handed Date": {
...
//     },
//     "Work Order#": {
//       fieldName: "RegulatoryTrxnFeeItem.Work_Order_Number__c",
//       type: "text"
//     },
//     Date: {
//       fieldName: "RegulatoryTrxnFeeItem.CreatedDate",
...
handleTransactionReset() {
  ...
  async handleSearch() {
    ...
    try {
      ...
      if (this.selectReport === "Aging Report") {
        ...
        const childComponent = this.template.querySelector(
          "c-finsys-daily-settlement-report"
        );
        if (childComponent) {
          childComponent.receiveFormFields(this.formFields);
          console.log("Form fields passed to child:", this.formFields);
        ...
        const childComponent = this.template.querySelector(
          "c-finsys-credit-balance-report"
        );
        if (childComponent) {
          childComponent.receiveFormFields(this.formFields);
          console.log("Form fields passed to child:", this.formFields);
        ...
        const childComponent = this.template.querySelector(
          "c-cumulative-deposit-summary-report"
        );
        if (childComponent) {
        ...
        const childComponent = this.template.querySelector(
          "c-daily-transaction-listing-report"
        );
        if (childComponent) {
          childComponent.receiveFormFields(
            this.formFields,
        ...
        const childComponent = this.template.querySelector(
          "c-returned-checks-summary-report"
        );
        if (childComponent) {
          childComponent.receiveFormFields(this.formFields);
          console.log(
        ...
        const childComponent = this.template.querySelector("c-core-c-t-report");
        if (childComponent) {
          childComponent.receiveFormFields(
            this.formFields,
            this.selectedActivities,
  ...
  get checkReportType() {
    return {
      ...
      isCumulativeDepositSummary:
        this.selectReport === "Cumulative Deposit Summary",
      isDailyTransactionListing:
        this.selectReport === "Daily Transaction Listing",
      isDepositSummary: this.selectReport === "Deposit Summary",
      isReturnedChecksSummary: this.selectReport === "Returned Checks Summary",
      isRefundedRequestHistory:
        this.selectReport === "Refunded Request History",
      isUserCloseoutReport: this.selectReport === "User Closeout Report",
      isNotaryPublicReconciliationReport:
        this.selectReport === "Notary Public Reconciliation Report",
  ...
  async handleDownloadPdf() {
    ...
    try {
      if (this.selectReport === "Daily Transaction Listing") {
        ...
        const pdfgenerator = this.template.querySelector(
          "c-pdf-generator-finsys-report"
        );
        if (pdfgenerator) {
          pdfgenerator.dailyTransactionReport(searchParams, fileName);
        } else {
        ...
        let searchParams = {
            WorkOrderNum: this.formFields['Work Order Number'] || '',
            name: this.formFields['Customer Name'] || '',
            numCheck: this.formFields['Check Number'] || '',
            dateCheck: this.formFields['Check Date'] || '',
            amountCheck: this.formFields['Check Amount'] || '',
            statusCheck: this.formFields['Status'] || '',
            reasonReturnCheck: this.formFields['Reason for Return'] || ''
        };
        const pdfgenerator = this.template.querySelector(
          "c-pdf-generator-finsys-report"
        );
        if (pdfgenerator) {
          pdfgenerator.returnCheckSummaryReport(searchParams, fileName);
        } else {
          console.error("Pdf generator component not found");
        }
      }
      else if (this.selectReport === "Refunded Request History") {
        const fileName = "Refunded_Request_History_Report";
        let searchParams = {
            refundDate: this.formFields['Refund Date'] || '',
            name: this.formFields['Customer Name'] || '',
            reasonRefund: this.formFields['Refund Reason'] || '',
            WorkOrderNum: this.formFields['Work Order'] || '',
            refundStatus: this.formFields['Refund Status'] || '',
            requestedBy: this.formFields['Requested By'] || '',
            paymentNum: this.formFields['Payment Number'] || '',
            paymentAmount: this.formFields['Payment Amount'] || '',
            refundAmount: this.formFields['Refund Amount'] || ''
        };
        const pdfgenerator = this.template.querySelector(
          "c-pdf-generator-finsys-report"
        );
        if (pdfgenerator) {
          pdfgenerator.refundRequestReport(searchParams, fileName);
        } else {
          console.error("Pdf generator component not found");
        }
      }
        ...
        if (pdfgenerator) {
          pdfgenerator.notaryPublicReport(searchParams, fileName);
        } else {
          console.error("Pdf generator component not found");
        }
      } else if (this.selectReport === "CORE-CT Deposit Summary" && !this.isRegularDeposits) {
        const fileName = "CORE-CT_Deposit_Summary_Report";
        let searchParams = {};
        const pdfgenerator = this.template.querySelector(
          "c-pdf-generator-finsys-report"
        );
        if (pdfgenerator) {
          pdfgenerator.coreCreditCardReport(searchParams, fileName);
        } else {
          console.error("Pdf generator component not found");
        }
      } else if (this.selectReport === "CORE-CT Deposit Summary" && this.isRegularDeposits) {
        ...
        if (pdfgenerator) {
          pdfgenerator.coreRegularDepositReport(searchParams, fileName);
        } else {
          console.error("Pdf generator component not found");
        }
      } else if (this.selectReport === "Deposit Summary") {
        ...
        if (pdfgenerator) {
          pdfgenerator.depositSummaryReport(searchParams, fileName);
        } else {
          console.error("Pdf generator component not found");
        }
      } else if (this.selectReport === "Cumulative Deposit Summary") {
        ...
        if (pdfgenerator) {
          pdfgenerator.cumulativeDepositSummaryReport(searchParams, fileName);
        } else {
          console.error("Pdf generator component not found");
        }
      } else if (this.selectReport === "User Closeout Report") {
        const fileName = "User_Closeout_Report";
        let searchParams = {};
        const pdfgenerator = this.template.querySelector(
          "c-pdf-generator-finsys-report"
        );
        if (pdfgenerator) {
          pdfgenerator.userCloseoutReport(searchParams, fileName);
        } else {
          console.error("Pdf generator component not found");
        }
      }
