...
import searchApplications from '@salesforce/apex/SAP_StateSealApplicationController.searchApplications';
import searchApplicationCount from '@salesforce/apex/SAP_StateSealApplicationController.searchApplicationCount';
import sap_stateSealM from '@salesforce/resourceUrl/sap_stateSealM';
import getCities from  '@salesforce/apex/SAP_StateSealApplicationController.getCities';
    ...
    return searchApplications({ searchCriteriaJson })
        .then(result => {
            ...
            this.paginatedResult = result.map(record => {
                return {
                    ...record,
                    formattedDate: this.formatDate(record.SAP_Date_of_SOTS_Response__c),
                    dispositionClass: this.getDispositionClass(record.SAP_Disposition__c)
                };
            });         

            this.showResults = this.paginatedResult.length > 0;
            this.isPagination = this.paginatedResult.length > 10;
...
// Handle the search button click
    ...
    get sortIcons() {
        return {
            SAP_Last_Name__c: this.sortedBy === 'SAP_Last_Name__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            SAP_First_Name__c: this.sortedBy === 'SAP_First_Name__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            SAP_Address_Line_1__c: this.sortedBy === 'SAP_Address_Line_1__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            SAP_City__c: this.sortedBy === 'SAP_City__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            SAP_Date_of_SOTS_Response__c: this.sortedBy === 'SAP_Date_of_SOTS_Response__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            SAP_Disposition__c: this.sortedBy === 'SAP_Disposition__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown'
        };
    }
    ...
    filterDataByDate() {
        ...
        this.filteredDataList = this.fullDataList.filter(record => {
            const responseDate = new Date(record.SAP_Date_of_SOTS_Response__c);
    ...
    async openSealAddModal(){
        ...
        //     const result = await sap_StateSealModal.open({
        //         size: 'small',
        ...
        try {
            ...
            this[NavigationMixin.Navigate]({
                ...
                attributes: {
                    componentName: 'c__sap_StateSealModal'  // The target component name
                },
                state: {
                  c__record: '',
                  c__mode:'addnew'
   ...
   // Handle pagination: Next page
    ...
    get sortedClassLastName(){
        return `slds-is-sortable ${this.sortedBy === 'SAP_Last_Name__c' ? 'sorted' : ''}`;

    }
     get sortedClassFirstName(){
        return `slds-is-sortable ${this.sortedBy === 'SAP_First_Name__c' ? 'sorted' : ''}`;
     }

    get sortedClassAddress1Name(){
        return `slds-is-sortable ${this.sortedBy === 'SAP_Address_Line_1__c' ? 'sorted' : ''}`;
    }

    get sortedClassCityName(){
        return `slds-is-sortable ${this.sortedBy === 'SAP_Date_of_SOTS_Response__c' ? 'sorted' : ''}`;
    }

    get sortedClassDateName(){
        return `slds-is-sortable ${this.sortedBy === 'SAP_City__c' ? 'sorted' : ''}`;
    }

    get sortedClassDispositionName(){
        return `slds-is-sortable ${this.sortedBy === 'SAP_Disposition__c' ? 'sorted' : ''}`;
    }
