...
import getPublicOfficial from '@salesforce/apex/PublicOfficialExcelController.getPublicOfficial';
import getElectionOffices from '@salesforce/apex/PublicOfficialExcelController.getElectionOffices';
import getPublicElectionOfficial from '@salesforce/apex/PublicOfficialExcelController.getPublicElectionOfficial';
import getElectionOfficesRedristrict from '@salesforce/apex/PublicOfficialExcelController.getElectionOfficesRedristrict';
import legislativeData from '@salesforce/apex/PublicOfficialExcelController.legislativeData';
    ...
    exportDataToExcelLegislative(columns, searchCriteria, fileName) {
        ...
        legislativeData({ searchCriteriaJson })
            .then(result => {
                ...
                this.paginatedResult = result.map(record => {
                    return {
                        Title: record.Legislator_Title__c || '',
                        Party: record.Party__c || '',
                        LastName: record.LastName || '',
                        FirstName: record.FirstName || '',
                        MiddleName: record.MiddleName || '',
                        District: record.DistrictID__c || ''
                    };
                });
    ...
    exportDataToExcelElectionOffice(columns, searchCriteria, fileName) {
        ...
        getElectionOffices({ searchCriteriaJson })
            .then(result => {
                ...
                this.paginatedResult = result.map(record => {
                    return {
                        //officeId: record.office.Id__c || '',
                        officeId: record.office.Id__c != null && String(record.office.Id__c).length < 3 
                            ? String(record.office.Id__c).padStart(3, '0')
                            : String(record.office.Id__c), // Pad Id__c to ensure length of 3
                        Office: record.office.Name__c,
                        emailAddress: record.office.Business_Email__c || '',
                        rovName: record.office.ROV_Name__c || '',
                        Address: record.office.Mailing_Address_Line_1__c || '',
                        State: record.office.Mailing_Address_State__c || '',
                        Zip: record.office.Mailing_Address_Zip__c || '',
                        Country: record.office.Mailing_Address_Country__c || '',
                        ElectionHeld: record.office.Election_Held_In__c || '',
                        City: record.office.Mailing_Address_City__c || '',
                        ...
                        Title: record.office.ROV_Name__c
                    };
                });
