...
import ELECTED_FIELD from '@salesforce/schema/Contact.Elected__c'; // Import Elected__c field
import upsertContact from '@salesforce/apex/AddOfficialContactController.upsertContact'; // Import upsertContact method
import fetchContactData from '@salesforce/apex/AddOfficialContactController.fetchContactData'; // Import fetchContactData method
        ...
        fetchContactData({ recordId: this.recordId })
            .then((contact) => {
                ...
                this.formData = {
                    ...
                    personalName: this.toTitleCase(contact.Personal_Name__c || ''),
                    title: contact.Title__c || '',
                    party: contact.Party__c || '',
                    apo: contact.Authorized_Public_Official__c || false,
                    sbt: contact.Office__c || '',
                    email: contact.Email || '',
                    fax: contact.Fax || '',
                    homePhone: contact.HomePhone || '',
                    businessPhone: contact.Phone || '',
                    extension: contact.Extension__c || '',
                    address1: this.toTitleCase(contact.MailingStreet || ''),
                    address2: this.toTitleCase(contact.MailingAddress2__c || ''),
                    city: (contact.MailingCity || '').toUpperCase(),
                    state: (contact.MailingState || '').toUpperCase(),
                    zipCode: contact.MailingPostalCode || '',
                    country: contact.MailingCountry || '',
                    elected: contact.Elected__c || '',
                    startTerm: contact.Start_Term__c || '',
                    endTerm: contact.End_Term__c || '',
                    indefiniteTerm: contact.Indefinite_Term__c || false,
                };
