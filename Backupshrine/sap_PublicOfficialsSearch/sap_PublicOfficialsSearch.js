...
import getPublicOfficial from '@salesforce/apex/SAP_PublicOfficialController.getPublicOfficial';
import deleteContact from '@salesforce/apex/SAP_PublicOfficialController.deleteContact'; 
import getPublicOfficialsCount from '@salesforce/apex/SAP_PublicOfficialController.getPublicOfficialsCount';
...
export default class PublicOfficialsSearch extends NavigationMixin(LightningElement) {
    ...
    @track sortedBy = 'Office__r.SAP_Id_Number__c'; // Empty initially
    ...
    connectedCallback() {
        ...
        // Fetch the SAP_Office__c records and populate the combobox options
        fetchOffices()
        .then((data) => {
    ...
    loadPublicOfficials() {
        ...
        const searchCriteria = {
            ...
            sortedBy: this.sortedBy || 'Office__r.SAP_Id_Number__c',
        ...
        .then(result => {
            ...
            this.paginatedResult = result.map(record => {
                return {
                    ...record,
                    Office: record.Office__c ? record.Office__r.SAP_Name__c : '',
    ...
    get sortIcons() {
        return {
            SAP_Office__c: this.sortedBy === 'Office__r.SAP_Id_Number__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            SAP_Title__c: this.sortedBy === 'SAP_Title__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            SAP_Party__c: this.sortedBy === 'SAP_Party__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            ...
            SAP_Start_Term__c: this.sortedBy === 'SAP_Start_Term__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            SAP_End_Term__c: this.sortedBy === 'SAP_End_Term__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown',
            ...
            SAP_Extension__c: this.sortedBy === 'SAP_Extension__c' ? (this.sortedDirection === 'asc' ? 'utility:arrowdown' : 'utility:arrowup') : 'utility:arrowdown'
            
        };
    }

    get sortedClassOffice(){
        return `slds-is-sortable ${this.sortedBy === 'Office__r.SAP_Id_Number__c' ? 'sorted' : ''}`;

    }

    get sortedClassOfficialTitle(){
        return `slds-is-sortable ${this.sortedBy === 'SAP_Title__c' ? 'sorted' : ''}`;

    }

    get sortedClassParty(){
        return `slds-is-sortable ${this.sortedBy === 'SAP_Party__c' ? 'sorted' : ''}`;
    ...
    get sortedClassStartTerm(){
        return `slds-is-sortable ${this.sortedBy === 'SAP_Start_Term__c' ? 'sorted' : ''}`;
    }

    get sortedClassEndTerm(){
        return `slds-is-sortable ${this.sortedBy === 'SAP_End_Term__c' ? 'sorted' : ''}`;
    }
    ...
    get sortedClassExtension(){
        return `slds-is-sortable ${this.sortedBy === 'SAP_Extension__c' ? 'sorted' : ''}`;
    }
    ...
    applyFilters() {
        ...
        .then(result => {
            ...
            this.paginatedResult = result.map(record => {
                return {
                    ...record,
                    Office: record.Office__r.SAP_Name__c,
                    formattedStartDate: this.formatDate(record.SAP_Start_Term__c),
                    formattedEndDate: this.formatDate(record.SAP_End_Term__c),
                };
            });
    ...
    handleExportResultButtonClick() {
        let headers = [
            ...
            { label: 'Personal Name', fieldName: 'SAP_Personal_Name__c' },
            { label: 'Authorized Public Official', fieldName: 'SAP_Authorized_Public_Official__c' },
            { label: 'Email Address', fieldName: 'Email' },
            { label: 'Fax', fieldName: 'Fax' },
            { label: 'Elected', fieldName: 'SAP_Elected__c' },
            { label: 'Indefinite Term', fieldName: 'SAP_Indefinite_Term__c' },            
            ...
            { label: 'Suite/Apartment/Floor', fieldName: 'SAP_MailingAddress2__c' },
    ...
    //     const result = await sap_PublicOfficialModal.open({
    //         size: 'small',
    ...
    async handleNavigateToAdd(){
        ...
        this[NavigationMixin.Navigate]({
            ...
            state: {
                ...
                c__returnTo: 'c__sap_PublicOfficialsSearch',
