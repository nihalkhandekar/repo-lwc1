...
import getAssetFileBase64 from '@salesforce/apex/SAP_StateSealApplicationController.getAssetFileBase64';
...
export default class SendEmailModalSS extends LightningModal {
    ...
    async handleSendEmail() {
        ...
        try {
            // Fetch Seal Image if Selected
            if (this.includeSeal) {
                const sealBase64 = await getAssetFileBase64({ fileName: 'CTGreatSealLogo' });
            ...
            // Fetch Arm Image if Selected
            if (this.includeArm) {
                const armBase64 = await getAssetFileBase64({ fileName: 'CTArmsLogo' });
                if (armBase64) {
                    emailData.sealArmDocuments.push({
                        name: 'StateArm.png',
                        content: armBase64
