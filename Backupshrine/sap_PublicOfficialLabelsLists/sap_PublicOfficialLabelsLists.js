...
import getOfficesByRecordType from '@salesforce/apex/SAP_PublicOfficialPrintController.getOfficesByRecordType';
...
export default class PublicOfficialLabelsLists extends LightningElement {
    ...
    wiredTowns({ error, data }) {
        if (data) {
            this.towns = data.map(office => ({ Id: office.Id, Name: office.SAP_Name__c }));
    ...
    wiredCities({ error, data }) {
        if (data) {
            this.cities = data.map(office => ({ Id: office.Id, Name: office.SAP_Name__c }));
    ...
    wiredStates({ error, data }) {
        if (data) {
            this.states = data.map(office => ({ Id: office.Id, Name: office.SAP_Name__c }));
    ...
    handleGenerateLabel() {
        ...
        const labelGenerator = this.template.querySelector('c-sap_-public-official-print');
        if (labelGenerator) {
