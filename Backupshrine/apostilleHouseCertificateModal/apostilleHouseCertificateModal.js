...
import getDocumentChecklistItem from '@salesforce/apex/ApostilleLetterController.getDocumentChecklistItem';
...
export default class ApostilleHouseCertificateModal extends LightningModal {
    ...
    async fetchChecklistItems() {
        try {
            ...
            if (response && response.length > 0) {
                ...
                this.checklistData = {
                    certificateNumber: checklistItem.Certificate_Number__c
                        ? checklistItem.Certificate_Number__c.toUpperCase()
                        : '',
                    signedBy: checklistItem.Signed_By__r?.Name
                        ? checklistItem.Signed_By__r.Name.toUpperCase()
                        : '',
                    signedStamp: checklistItem.Signed_By__r?.Seal_Stramp_of__c
                        ? checklistItem.Signed_By__r.Seal_Stramp_of__c.toUpperCase()
                        : '',
                    position: checklistItem.Signed_By__r?.Position__c
                        ? checklistItem.Signed_By__r.Position__c.toUpperCase()
                        : '',
                    ...
                    Signing_Authority_Name: checklistItem.Signing_Authority__r?.Name
                        ...
                        : '',
                    Signing_Authority_Title: checklistItem.Signing_Authority__r?.Staff_Title__c
                        ? this.toTitleCase(checklistItem.Signing_Authority__r.Staff_Title__c)
                        : '',
    ...
    handlePrint() {
        ...
        const pdfgenerator = this.template.querySelector('c-apostille-pdf-generator'); // Adjust selector as needed
