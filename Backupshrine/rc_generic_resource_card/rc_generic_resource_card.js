...
export default class Rc_generic_resource_card extends LightningElement {
    passiveHeartIcon = assetFolder + "/icons/RC/heart-passive.svg";
    activeHeartIcon = assetFolder + "/icons/RC/heart-active.svg";
    noImage = assetFolder + "/icons/RC/no-image.svg";
    addCollectionIcon = assetFolder + "/icons/RC/blue-circle-plus.png";
    ...
    set newlist(value) {
        this._newlist = value;
        this.collectionsList = JSON.parse(JSON.stringify(this.newlist))
    }
    @api
    get resources() {
    ...
    set resources(value) {
        ...
        this.updatedResources = this.resources;
        // this.collectionsList = JSON.parse(JSON.stringify(this.newlist))
    }
    connectedCallback() {
    ...
    getCollectionInfo() {
        getAllCollections()
            ...
            .catch((error) => {
                ComponentErrorLoging("rc_generic_resource_card", 'getAllCollections', '', '', 'High', error.message);
            });
    }
    handleAddResource(event) {
        try {
            ...
            addResource({
                ...
                ComponentErrorLoging("rc_generic_resource_card", 'addResource', '', '', 'High', error.message);
            });
            ...
            ComponentErrorLoging("rc_generic_resource_card", 'handleAddResource', '', '', 'Medium', error.message);
        }
    }
    ...
    handleCollectionsPopup(event) {
        try {
            ...
            ComponentErrorLoging("rc_generic_resource_card", 'handleCollectionsPopup', '', '', 'High', error.message);
        }
    }
    handleDeSelect(event) {
        ...
        removeResource({
         ...
         const resourceAddedEvent = new CustomEvent("resourceremoved", {
            detail: id
        });
         this.dispatchEvent(resourceAddedEvent);
            }
        }).catch((error) => {
            ComponentErrorLoging("rc_generic_resource_card", 'removeResource', '', '', 'High', error.message);
        });
        let alltitle = this.template.querySelectorAll('.slds-popover');
        if (alltitle.length) {
            alltitle.forEach(element => {
    ...
    updateViewState(event) {
        ...
        setViewStatKnowledgeRecords({
            ...
            ComponentErrorLoging("rc_generic_resource_card", 'setViewStatKnowledgeRecords', '', '', 'High', error.message);
        });
		let targetText = event ? event.target.textContent : "Resource card";
        this.insertAnalyticsEvent("Popular Resources", "", targetText);
    }
	...
	insertAnalyticsEvent(sectiontitle, targetVal, targetText) {    
        insertRecord(null, "rclanding", sectiontitle, "", sectiontitle, 
          "Resource card click", targetVal, targetText, this.startTime, new Date().getTime()
        );
      }
    ...
    openShareModal(event) {
        ...
        let resourceContainer = this.template.querySelector("[data-id= resourceCard ]");
