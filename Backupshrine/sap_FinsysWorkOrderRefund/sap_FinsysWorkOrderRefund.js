...
import getWorkOrderApplication from '@salesforce/apex/SAP_FinsysRefundController.getWorkOrderApplication';
import getApplicationsCount from '@salesforce/apex/SAP_FinsysRefundController.getApplicationsCount';
...
export default class FinsysWorkOrderRefund extends NavigationMixin(LightningModal) {
    ...
    formatTableData(result) {
        ...
        return result.map(row => ({
            ...row,
            LastName: row.SAP_Last_Name__c || '-',
            FirstName: row.SAP_First_Name__c || '-',
            Id: row.Id
        }));
    }
