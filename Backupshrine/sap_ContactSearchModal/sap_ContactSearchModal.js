...
import searchContacts from '@salesforce/apex/SAP_DocumentChecklistJsonGenerator.searchContacts';
import updateContactSealStamp from '@salesforce/apex/SAP_DocumentChecklistJsonGenerator.updateContactSealStamp';
...
export default class ContactSearchModal extends LightningModal {
        ...
        try {
            ...
            this.contacts = result.map(contact => {
                ...
                const updatedTermHistory = contact.termHistory.map(term => ({
                    ...term,
                    SAP_Start_Term__c: term.SAP_Start_Term__c === "Indefinite Term" ? "N/A" : term.SAP_Start_Term__c,
                }));
    ...
    handleInputChange(event) {
        const contactId = event.target.dataset.contactid; // Get Contact ID
        const fieldValue = event.target.value; // Get input value
        const fieldName = event.target.name; // Field Name

        console.log(`Entered Value: ${fieldValue}, Contact ID: ${contactId}, Field Name: ${fieldName}`);
        ...
        this.contacts = this.contacts.map(contact => {
            ...
            const updateData = {
                Id: contactId,
                SAP_Seal_Stramp_of__c: fieldValue
            };

            console.log('Updating Contact:', JSON.stringify(updateData));

            // Call Apex method to update the contact
            updateContactSealStamp({ contactJson: JSON.stringify(updateData) })
                .then(() => {
