...
import processLabelSelections from '@salesforce/apex/SAP_PublicOfficialLabelService.processLabelSelections';
import getTownClerksByTownCity from '@salesforce/apex/SAP_PublicOfficialLabelService.getTownClerksByTownCity';
import getRegistrarsByTownCity from '@salesforce/apex/SAP_PublicOfficialLabelService.getRegistrarsByTownCity';
import getMayorByTownCity from '@salesforce/apex/SAP_PublicOfficialLabelService.getMayorByTownCity';
import getLegislatorsByAddressAndType from '@salesforce/apex/SAP_PublicOfficialLabelService.getLegislatorsByAddressAndType';
import getROVLabels from '@salesforce/apex/SAP_PublicOfficialLabelService.getROVLabels';
import getOutOfStateLabels from '@salesforce/apex/SAP_PublicOfficialLabelService.getOutOfStateLabels';
...
export default class PublicOfficialPrint extends LightningElement {
    ...
    generate5160Label(labelData) {
        try {
            ...
            labelData.forEach((label, index) => {
                ...
                let postalCode = (label.postalCode || '').toUpperCase(); // Convert to uppercase

                            // Ensure length is at least 5 by padding with leading zeros
    ...
    generateLegislatorPDF(labelData) {
        ...
        function renderRowData(row) {
            ...
            let postalCode = (row.postalCode || '').toUpperCase(); // Convert to uppercase
    ...
    generateLandscapePDFTest(labelData, selectedTitle) {
        // Define the capitalizeWords helper function within the scope
        function capitalizeWords(str) {
        ...
        try {
            ...
            const sectionSpacing = 0.1;
            let currentY = yMargin;
            let currentX = xMargin;
            let currentPage = 1;
    
            // Define headers and column widths for each label type
            const formats = {
                Default: {
            ...
            // Set line width, font, and size for headers
            doc.setLineWidth(0.0039);
            doc.setFont('helvetica', 'bolditalic');
            ...
            // Function to add footer with date and page number
            function addFooter() {
                doc.line(xMargin, pageHeight - 0.7, pageWidth - xMargin, pageHeight - 0.7);
            ...
            // Function to render row data with wrapping and alignment
            function renderRowData(row, showTown) {
                ...
                // Apply capitalizeWords function to selected fields
                const formattedRow = {
                    officeName: showTown ? capitalizeWords(row.officeName || '') : '', // Only show Town once per section
                    name: capitalizeWords(row.name || ''),
                    addressLine1: capitalizeWords(row.addressLine1 || ''),
                    addressLine2: capitalizeWords(row.addressLine2 || ''),
                    city: capitalizeWords(row.city || ''),
                ...
                // Collect row data with conditional spacing
                const rowData = [
                    selectedTitle !== 'Legislator' ? doc.splitTextToSize(formattedRow.officeName, colWidths[0]) : '', // Town
                    selectedTitle !== 'ROV' ? doc.splitTextToSize(formattedRow.name, colWidths[selectedTitle === 'Legislator' ? 0 : 1]) : '', // Name
                    doc.splitTextToSize(formattedRow.addressLine1, colWidths[2]), // Address1
                    doc.splitTextToSize(formattedRow.addressLine2, colWidths[3]), // Address2
                    doc.splitTextToSize(`${formattedRow.city} ${formattedRow.state} ${formattedRow.postalCode}`, colWidths[4]), // City/State/Zip
                    doc.splitTextToSize(formattedRow.phone, colWidths[5]), // BusPhone
                    doc.splitTextToSize(formattedRow.fax, colWidths[6]), // Fax
                    doc.splitTextToSize(formattedRow.email, colWidths[7]), // Email
                    doc.splitTextToSize(formattedRow.party, colWidths[8]) // Party
                ];
    
                // Render town name on its own line if applicable
                if (showTown && formattedRow.officeName) {
                    doc.setFont('helvetica', 'bold'); // Set font to bold for the Town name
                    doc.setFontSize(9);
                    doc.text(formattedRow.officeName, xMargin, currentY); // Render the town name
                    doc.setFont('helvetica', 'normal'); // Reset font to normal for the rest of the row
                    currentY += lineSpacing; // Move down to the next line for other row details
                }
    
                // Determine the maximum number of lines in this row (excluding town name)
                const maxLines = Math.max(...rowData.slice(1).map(data => data ? data.length : 0));
    
                // Render each line of the row (excluding town)
                for (let i = 0; i < maxLines; i++) {
                    currentX = xMargin + colWidths[0]; // Start rendering after the town column
                    rowData.slice(1).forEach((data, index) => {
                        doc.setFontSize(8);
                        doc.text(data[i] || '', currentX, currentY); // Render the content for each column
                        currentX += colWidths[index + 1]; // Move X position for the next column
                    });
                    currentY += lineSpacing; // Move down to the next line in the row
                }
                currentY += sectionSpacing; // Add extra space between rows
            }
    
            // Iterate over data and add rows
            let currentTown = '';
            labelData.forEach((row) => {
                if (currentY > pageHeight - 1.0) { // Check if we need a new page
                ...
                // Show the town name only once for each new town
                const showTown = row.officeName && row.officeName !== currentTown && selectedTitle !== 'Legislator';
                if (showTown) {
                    currentTown = row.officeName; // Update the current town
                }
    
                doc.setFont('helvetica', 'normal'); // Reset to normal font for data rows
                renderRowData(row, showTown); // Render row data with showTown flag
            });
