...
import uploadPdfToIndividualApplication from '@salesforce/apex/PdfUploadController.uploadPdfToIndividualApplication';
import uploadPdfToIndividualApp from '@salesforce/apex/PdfUploadController.uploadPdfToIndividualApp';
// import fetchPdfUrlFromDatabase from '@salesforce/apex/PdfUploadController.fetchPdfUrlFromDatabase';
import checkIfDocumentExists from '@salesforce/apex/PdfUploadController.checkIfDocumentExists';
...
export default class PdfGenerator extends LightningElement {
    ...
    async generatePdfDocumentLatest(type){
       ...
       // centerText('Secretary of the State of Connecticut', currentY, 24, 'normal');
        currentY += 0.25;
        centerText('APOSTILLE', currentY, 14, 'bold');
        currentY += 0.2;
        centerText('(Convention de La Haye du 5 octobre 1961)', currentY, 11.5, 'normal');
        currentY += 0.5;
       ...
       // doc.addImage(sealLogo, 'PNG', sealLogoX, sealLogoY, sealLogoWidth, sealLogoHeight);
        ...
        doc.setTextColor(0,0,0) // Dark blue color
        const englishFontSize = 11.5;
        const spanishFontSize = 10;
        const englishLineSpacing = 0.150;
        const spanishLineSpacing = 0.28;
        ...
        const signedStamp = this.checkList.signedStamp || '';
        ...
        doc.setFont("times", "normal");
        doc.text(country, centerX-1, currentY);
        ...
        doc.setFont("times", "normal");
        doc.text(signedBy, centerX-1, currentY);
        currentY += englishLineSpacing;
        doc.setFont("times", "italic");
        doc.setFontSize(spanishFontSize);
        ...
        wrappedPositionText.forEach((line) => {
            doc.text(line, centerX-1, wrapY); // Adjust horizontal alignment
        ...
        // Wrap the position text
        const wrappedStampText = doc.splitTextToSize(signedStamp, availableWidth);
        doc.setFont("times", "normal");
        let wrapStampY = currentY;
        // Print the wrapped position text
        wrappedStampText.forEach((line) => {
            doc.text(line, centerX-1, wrapStampY); // Adjust horizontal alignment
            wrapStampY += englishLineSpacing;
        });
        currentY += englishLineSpacing;
        doc.setFont("times", "italic");
        doc.setFontSize(spanishFontSize);
        ...
        const nameWidth = doc.getTextWidth(Signing_Authority_Name);
        doc.setFont("times", "bold");
        doc.text(Signing_Authority_Title, centerX - 1.5 + nameWidth + 0.05, currentY);
        currentY += englishLineSpacing;
        doc.setFont("times", "italic");
        doc.setFontSize(spanishFontSize);
        ...
        doc.line(centerX + 0.18, currentY-0.25 , centerX + 3, currentY-0.25); 
